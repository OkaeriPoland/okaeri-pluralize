...
def generate(data):
    ...
    for num, match in enumerate(matches, start=1):
        ...
        if algorithm == "(n != 1)": algorithm = "(n == 1) ? 0 : 1"
        if algorithm == "(n != 0)": algorithm = "(n == 0) ? 0 : 1"
        if "?" not in algorithm and "0" is not algorithm: algorithm = f"{algorithm} ? 1 : 0"
        print(f"add(\"{match.group(1)}\", {match.group(3)}, n -> {algorithm}); // {match.group(2)}")
